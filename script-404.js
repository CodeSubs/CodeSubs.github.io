...
function init() {
  ...
  if (window.innerWidth < 450) {
    // Constrain by both width and height to prevent vertical overflow on small/landscape screens
    fontSize = Math.min(window.innerWidth / 2.5, window.innerHeight * 0.35, 300);
  } else {
    fontSize = Math.min(window.innerWidth / 4, window.innerHeight * 0.40, 300);
  }
  ...
  if (ctx2) {
    ...
    // Responsive font size for Arabic text
    let labelFontSize;
    if (window.innerWidth < 450) {
      // Constrain by both width and height (e.g. landscape mobile)
      labelFontSize = Math.max(20, Math.min(window.innerWidth * 0.1, window.innerHeight * 0.1, 70)); 
    } else {
      labelFontSize = Math.max(16, Math.min(window.innerWidth * 0.06, window.innerHeight * 0.08, 56));
    }
    ...
    ctx2.shadowColor = 'rgba(255, 215, 0, 0.25)';
    ctx2.shadowBlur = 18;
    ctx2.font = '300 ' + Math.floor(labelFontSize) + 'px "Expo Arabic Light", Arial';

    // Responsive positioning for the Arabic text relative to the main 404
    // Place it below the 404 text (center + half font size + margin)
    const labelY = (window.innerHeight / 2) + (fontSize * 0.5) + 20;

    ctx2.fillText('الصفحة غير موجودة', window.innerWidth / 2, labelY);
    ctx2.shadowBlur = 0;
  }

  // Responsive positioning for the UI Layer (Button)
  const uiLayer = document.querySelector('.ui-layer');
  if (uiLayer) {
    // Keep button at a safe distance from bottom
    uiLayer.style.bottom = (window.innerWidth < 450) ? '20%' : 'clamp(118px, 8vh, 64px)';
    
    // Scale button down on short screens to prevent overcrowding
    // Preserves CSS translateX(-50%) centering
    if (window.innerHeight < 450) {
       const scale = Math.max(0.7, window.innerHeight / 500);
...
if (document.fonts && document.fonts.load) {
  document.fonts.load('16px "Expo Arabic Light"').then(run).catch(run);
} else {
  run();
}
